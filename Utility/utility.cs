...
public static class Utility
{
    ...
    {
        return input[pos + 3] + ((uint)input[pos + 2] << 8) + ((uint)input[pos + 1] << 16) + ((uint)input[pos] << 24);
    }

    public static ulong ReadUInt64BigEndian(byte[] input, int pos)
    {
        return input[pos + 7] + ((ulong)input[pos + 6] << 8) + ((ulong)input[pos + 5] << 16) + ((ulong)input[pos + 4] << 24) + ((ulong)input[pos + 3] << 32) + ((ulong)input[pos + 2] << 40) + ((ulong)input[pos + 1] << 48) + ((ulong)input[pos] << 56);
    }

    public static float ReadSingleBigEndian(byte[] input, int pos)
    {
        byte[] bytes = { input[pos + 3], input[pos + 2], input[pos + 1], input[pos] };
    ...
    public static float ReadFloat(byte[] source, int offset, bool isBigEndian)
    {
        ...
        if (!BitConverter.IsLittleEndian)
        {
            ...
            byte[] bytes = { source[offset + 3], source[offset + 2], source[offset + 1], source[offset] };
            return BitConverter.ToSingle(bytes, 0);
        }
