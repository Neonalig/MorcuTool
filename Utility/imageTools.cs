...
public static class imageTools
{
    ...
    public static List<byte> ConvertToNintendoTPL(string filename, byte[] file)
    {
            var output = new List<byte>();
                ...
                if (AppState.activePackage.packageType == Package.PackageType.SkyHeroes && Utility.ReverseEndian(BitConverter.ToUInt32(file,pos)) != 0)        //skip those annoying extra headers from MySims SkyHeroes
                {
                    startoffile = 0x50;
                }
            ...
            switch (AppState.activePackage.packageType)
            {
                ...
                case Package.PackageType.SkyHeroes:
                    magic = Utility.ReverseEndian(BitConverter.ToUInt32(file,pos));
                    pos += 4;
                    flags = Utility.ReverseEndian(BitConverter.ToUInt32(file, pos));
                    pos += 4;
                ...
                if (AppState.activePackage.packageType == Package.PackageType.SkyHeroes)
                {
    ...
    public static void TPLToMySimsTPL(string filename)
    {
        ...
        var imagesubblocks = new List<byte[]>();
        ...
        using var writer = new BinaryWriter(File.Open($"{filename}2.tpl", FileMode.Create));
