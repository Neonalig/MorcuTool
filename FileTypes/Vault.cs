...
public class Vault
{
    ...
    public readonly Dictionary<ulong, ulong> VaultHashesAndIndexes = new();
    public readonly Dictionary<ulong, string> VaultHashesAndFileNames = new();

    public readonly List<luaString> luaStrings = new();
    ...
    public void LoadVault()
    {
        ...
        {
            var newLuaString = new luaString
            {
                hash = Utility.ReverseEndian(reader.ReadUInt32()),
                nameOffset = Utility.ReverseEndian(reader.ReadUInt32()),
            };
        ...
        foreach (ulong hash in VaultHashesAndIndexes.Keys)
        {
            ...
            VaultHashesAndFileNames.Add(hash, Encoding.UTF8.GetString(bytes.ToArray(), 0, bytes.ToArray().Length));
        }

        if (AppState.activePackage.packageType == Package.PackageType.Agents) //then add the lua filenames that aren't in the vaults
        {
