...
public class TPLtexture
{
    ...
    public TPLtexture(Subfile basis) {
            ...
            if (AppState.activePackage.packageType == Package.PackageType.SkyHeroes && Utility.ReadUInt32BigEndian(basis.filebytes,pos) != 0)        //skip those annoying extra headers from MySims SkyHeroes
            {
                startoffile = 0x50;
            }
                        ...
                        for (int y = 0; y < height; y += 8)
                        {
                            for (int x = 0; x < width; x += 8)
                            {
                                for (int j = 0; j < 4; j++) {
                                    ...
                                    switch (j)
                                    {
                                        case 1 or 3 when ((x + 4) >= width):
                                        case 2 or 3 when ((y + 4) >= height):
                                            continue;
                                    }
